    ...
    <table class="table table-bordered table-striped table-actions">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:center;">
                    <?php 
                        $id_pro =  $data['OrderProduct']['id'];
                        $ID     =  $data['Order']['id'];
                        $qty    =  $data['OrderProduct']['qty']; 
                        $plus   =  ($qty+1);
                        $minus  =  ($qty-1);
                    ?>
                    <?php if ($qty != 1 && $qty != 0): ?>
                            <a href="<?php echo $settings['cms_url'].$ControllerName; ?>/UpdateQty/<?php echo $ID; ?>/<?php echo $id_pro; ?>/<?php echo $minus; ?>/" class="pull-left" onclick="OnClickSaveStay()">
                                <span class="glyphicon glyphicon-minus"></span>
                            </a>
                        <?php else: ?>
                            <a href="#" class="disabled pull-left" tabindex="-1" onclick="alert('Can not be reduced')"><span class="glyphicon glyphicon-minus"></span></a>
                    <?php endif ?>
                    <?php echo number_format($data['OrderProduct']['qty']); ?>
                    <a href="<?php echo $settings['cms_url'].$ControllerName; ?>/UpdateQty/<?php echo $ID; ?>/<?php echo $id_pro; ?>/<?php echo $plus; ?>/" class="pull-right" onclick="OnClickSaveStay()">
