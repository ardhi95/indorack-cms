            ...
            <table class="table table-bordered table-striped table-actions">
                <thead>
                    <tr>
                    	...
                    	<th>No</th>
                        <th>
                        	<?php echo $this->Paginator->sort("$ModelName.order_no",__('PO No.'),array("class"=>"sorting"));?>
                        </th>
                        
                        <th>
                        	<?php echo $this->Paginator->sort("Customer.fullname",__('Customer'),array("class"=>"sorting"));?>
                        </th>
                        
                        <th>
                        	<?php echo $this->Paginator->sort("Customer.fullname",__('Delivery Date'),array("class"=>"sorting"));?>
                        </th>

                        <th class="text-center">
                        	<?php echo $this->Paginator->sort("DeliveryType.name",__('Delivery By'),array("class"=>"sorting"));?>
                        </th>
                        <th>
                        	<?php echo $this->Paginator->sort("Order.is_assembling",__('Assembling'),array("class"=>"sorting"));?>
                        </th>
                        
                        <th>
                        	<?php echo $this->Paginator->sort("DeliveryStatus.name",__('Delivery'),array("class"=>"sorting","title"=>"Delivery Status"));?>
                        </th>
                        
                        <th class="text-center">
                        	<?php echo $this->Paginator->sort("AssemblyStatus.name",__('Assembling'),array("class"=>"sorting","title"=>"Assembling Status"));?>
                        </th>
                        
                        <th class="text-center">
                        	<?php echo $this->Paginator->sort("PickupStatus.name",__('Pickup'),array("class"=>"sorting","title"=>"Pickup  Status"));?>
                        </th>
                ...
                <tbody>
                    ...
                    <tr>
						...
						if(
								in_array($data["Order"]["delivery_status"],array(3,5,6,11))
							or
                        ...
                        <td>
                        	<?php  
                        		...
                        		if (!empty($data[$ModelName]['pickup_date'])) {
                        			# code...
                        			$datepic = $data[$ModelName]['pickup_date']; 
                        			echo date("d-m-Y H:i", strtotime($datepic));
                        		}else{
                        			$date = $data[$ModelName]['delivery_date']; 
                        ...
                        <td style="text-align: center;">
                        	<?php
                        		$driver		=	$this->General->IsEmptyVal($data["DeliveryType"]['name']);
                        		$check		=	$this->General->IsEmptyVal($data["DeliveryStatus"]['name']);
                        		if ($check == "Not Assign") {
                        			echo "-";
                        		}else{
                        			if ($driver == "Delivery by driver") {
                        				echo $data[0]['Driver'];
	                        		}else{
	                        			echo $this->General->IsEmptyVal($data["DeliveryType"]['name']);	
	                        		}
                        		}
                        	?>
                        </td>
                        <td class="col-md-1" style="text-align:center;">
							<?php
								...
								$assembly		=	$data[$ModelName]['is_assembling'];
                        		$check		=	$this->General->IsEmptyVal($data["AssemblyStatus"]['name']);
                        		if ($check == "Not Assign") {
                        			echo "-";
                        		}else{
                        			if ($assembly == "1") {
                        				echo $data[0]['Technitions'];
	                        		}else{
	                        			echo "No";	
	                        		}
                        		}
                        ...
                        <td class="text-center">
							...
							<?php if (
								...
								<a href="<?php echo $settings['cms_url'].$ControllerName?>/Edit/<?php echo $data[$ModelName]["id"]?>/<?php echo $page?>/<?php echo $viewpage?>" class="btn btn-warning btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('View')?>" <?php echo $disabledEdit?>><i class="fa fa-eye"></i></a>
							<?php else: ?>
